...
int is_tab(char symbol) { return symbol == '\t'; }

int is_end_of_line(char symbol) { return symbol == '\n'; }
...
void print_with_unprintable(char symbol) {
  if (0 <= symbol && symbol <= 31 && !is_tab(symbol) &&
      !is_end_of_line(symbol)) {
    symbol += 64;
    printf("^%c", symbol);
  } else if (symbol == 127) {
    symbol = '?';
...
void print_line(int *line_number, const char *line, int length,
                const Flags *flags, int *is_empty) {
...
void read_and_output_file_line_by_line(const char *filename,
  ...
  while (True) {
    ...
    line_actual_length =
        s21_getline_allocate(&line, &line_allocated_length, input_file);
...
int set_flags(int counter, const char **arguments, Flags *flags,
              int *flag_counter) {
  ...
  for (int argument_index = 0; argument_index < counter; ++argument_index) {
    ...
    if ((argument_length == short_flag_length &&
        ...
        (argument_length == get_string_length("--number-nonblank") &&
         are_equal("--number-nonblank", arguments[argument_index],
                   argument_length))) {
      flags->b = True;
      ++(*flag_counter);
      ...
      ++(*flag_counter);
               ...
               (argument_length == get_string_length("--number") &&
                are_equal("--number", arguments[argument_index],
                          argument_length))) {
      flags->n = True;
      ++(*flag_counter);
    } else if ((argument_length == short_flag_length &&
               ...
               (argument_length == get_string_length("--squeeze-blank") &&
                are_equal("--squeeze-blank", arguments[argument_index],
                          argument_length))) {
      flags->s = True;
      ++(*flag_counter);
    } else if (argument_length == short_flag_length &&
      ...
      flags->v = True;
      ++(*flag_counter);
    } else if (argument_length >= 1 && arguments[argument_index][0] == '-') {
...
int main(int counter, const char **arguments) {
  ...
  if (status) {
    for (int file_index = flag_counter + 1; file_index < counter;
         ++file_index) {
